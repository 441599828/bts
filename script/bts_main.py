...
# Dataset
parser.add_argument('--dataset', type=str, help='dataset to train on, Carla', default='nyu')
...
inv_normalize = transforms.Normalize(
    mean=[-0.133 / 0.187, -0.163 / 0.202, -0.186 / 0.215],
    std=[1 / 0.187, 1 / 0.202, 1 / 0.215]
)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    var_sum = [var.sum().detach().cpu() for var in model.parameters() if var.requires_grad]
    var_cnt = len(var_sum)
            ...
            if not args.do_online_eval and global_step and global_step % args.save_freq == 0:
                if not args.multiprocessing_distributed or (
                    ...
                    torch.save(checkpoint,
                               args.log_directory + '/' + args.model_name + '/model-{}'.format(global_step) + '.pt')
                ...
                if eval_measures is not None:
                    for i in range(9):
                        ...
                        if is_best:
                            ...
                            torch.save(checkpoint, args.log_directory + '/' + args.model_name + model_save_name + '.pt')
...
def main():
    ...
    if args.checkpoint_path == '':
        ...
        os.system(command)
        command = 'cp dataloader_whn.py ' + aux_out_path
        os.system(command)
    else:
